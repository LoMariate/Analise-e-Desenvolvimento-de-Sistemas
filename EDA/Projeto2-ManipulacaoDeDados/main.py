...
def top10ClubesCompras(dataFrame):
    topClubes = dataFrame['Team_to'].value_counts().head(10)
...
def top10Transferencias(dataFrame):
    dataFrame_sorted = dataFrame.sort_values(by='Transfer_fee', ascending=False).head(10).reset_index(drop=True)
    dataFrame_sorted.index += 1
    
    dataFrame_sorted = dataFrame_sorted.rename(columns={'Name': 'Nome', 'Team_to': 'Time', 'Transfer_fee': 'Transferência'})
    dataFrame_sorted['Transferência'] = dataFrame_sorted['Transferência'].map('${:,.2f}'.format)
...
def buscarJogadorNome(dataFrame, playerNome):
    players = dataFrame[dataFrame['Name'].str.contains(playerNome, case=False, na=False)].reset_index(drop=True)
    players.index += 1
...
# Função 5: Agrupar os jogadores por idade. Exibir a idade e o número de jogadores negociados em cada idade
